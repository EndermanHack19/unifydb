...
def __getattr__(name: str):
    ...
    if name in adapters:
        import importlib
        module = importlib.import_module(adapters[name], package="unifydb.adapters")
        return getattr(module, name)
    
    raise AttributeError(f"module 'unifydb.adapters' has no attribute '{name}'")
